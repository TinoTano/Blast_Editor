...
update_status ModuleInput::PreUpdate(float dt)
{
	...
	while(SDL_PollEvent(&e))
	{
		...
		switch(e.type)
		{
			...
			case SDL_DROPFILE:
			{
				std::string s_name(e.drop.file);
				if (s_name.find(".fbx") || s_name.find(".FBX"))
				{
					App->mesh_importer->ImportMesh(e.drop.file);
				}
				else if (s_name.find(".png") || s_name.find(".jpg") || s_name.find(".tga") || s_name.find(".dds"))
				{
					App->texture_importer->ImportTexture(e.drop.file);
				}
